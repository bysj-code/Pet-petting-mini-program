<template>
	<div class="page_root" id="root_demo">
		            ...
		            <div class="col-12">
		                <div class="sw_no">
							...
							<div class="card_notice">
								<div class="notice_title">通知公告 <router-link to="notice/list">更多</router-link></div>
								<!-- 公告组件 -->
		...
		<div class="warp">
		    <div class="container">
		        <div class="row">
					...
					<!-- 推荐宠物信息模块(开始) -->
		<div class="warp" v-if="$check_action('/pet_information/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		...
		<!-- 推荐宠物信息模块(结束) -->
					<!-- 推荐救助基地模块(开始) -->
		<div class="warp" v-if="$check_action('/rescue_base/list', 'get')">
		    <!-- 容器 -->
		    <div class="container">
		        <div class="row">
		            <div class="col-12">
		                ...
		                <div class="diy_recommend">
		                    ...
		                    <div class="more_box">
		                        <router-link to="/rescue_base/list" class="more">
		                            <span>更多</span>
		                        </router-link>
		                    </div>
		                    <div class="switch" @click="show_list_rescue_base = !show_list_rescue_base">
		                        <div class="switch_box">
		                            <span v-if="show_list_rescue_base"> 切换表格 </span>
		                            <span v-else> 切换列表 </span>
		                        </div>
		                    </div>
		                </div>
		                <!-- 救助基地推荐列表组件 -->
		                <list_rescue_base v-if="show_list_rescue_base" :list="list_rescue_base" />
		                <div class="overflow-auto" v-else>
		                    <table id="list_diy" role="table" aria-busy="false"
		                           :aria-colcount="fields_rescue_base.length"
		                           class="table b-table table-striped table-hover">
		                        <thead>
		                        <tr>
		                            <th v-for="(o,i) in fields_rescue_base" :key="i">
		                                {{o.label}}
		                            </th>
		                        </tr>
		                        </thead>
		                        <tbody>
		                        <tr v-for="(o, i) in list_table_rescue_base" :key="i" @click="to_details('rescue_base',o,'rescue_base_id')">
		                            <td v-for="(oj,n) in fields_rescue_base">
		                                <img v-if="oj.type && oj.type == '图片' " :src="$fullUrl(o[oj.key])" alt=""
		                                     v-default-img="'/img/default.png'">
										<span v-else-if="oj.type && oj.type == 'UID' ">
											<span style="display: none">
...
<script>
	import mixin from "@/mixins/page.js";
				...
				import list_rescue_base from "@/components/diy/list_rescue_base.vue";
	...
	export default {
			...
			return {
				...
				fields_pet_information: [
						...
						type: "下拉"
													...
													{
						key: "introduction_to_pets",
						label: "宠物简介",
						type: "多文本"
					},
									],
							show_list_rescue_base: true,
				list_rescue_base: [],
				vm_arr_rescue_base: [
					"rescue_base_id",
							"rescue_title",
			...
			get_pet_information() {
				let url = "~/api/pet_information/get_list?";
					...
					if (json.result) {
						...
						// 获取救助基地列表
			get_rescue_base() {
				let url = "~/api/rescue_base/get_list?";
										this.$get(url, {
					...
					if (json.result) {
						this.list_rescue_base = json.result.list;
					}
				})
			},
		...
		computed: {
		        ...
		        return list_table;
					    ...
					    list_table_pet_daily() {
		        var list = this.list_rescue_base;
		        var list_table = [];
		        for (let i = 0; i < list.length; i++) {
		            list_table[i] = {};
		            this.vm_arr_rescue_base.map((o) => {
		                // 第二个中括号是对象的属性
		                list_table[i][o] = list[i][o] || "";
		            });
		        }
