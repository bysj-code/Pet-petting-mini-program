<template>
  <view class="page_message_notification diy_table diy_table_tml" id="message_notification_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                      <uni-forms-item label="提醒日期" name="reminder_date">
                      <uni-easyinput type="text" v-model="query.reminder_date" placeholder="提醒日期" />
                    </uni-forms-item>
                  </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/message_notification/view?" v-show="user_group == '管理员' || $check_action('/message_notification/table','add') || $check_action('/message_notification/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="1 && $check_field('get','reminder_title')" class="col">
						<view class="view">
                            ...
                            <span>提醒标题</span>
								...
								<text>
									{{ o["reminder_title"] }}
													...
													<view v-if="0 && $check_field('get','remind_users')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>提醒用户</span>
                          </view>
								...
								<text>
									{{ get_user_remind_users(o['remind_users']) }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="0 && $check_field('get','reminder_date')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>提醒日期</span>
                          </view>
								...
								<text>
									{{ $toTime(o["reminder_date"],"yyyy-MM-dd") }}
								</text>
							</view>
										</view>
					</view>
													<view v-if="0 && $check_field('get','reminder_message')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>提醒信息</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/message_notification/table','del') || $check_action('/message_notification/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/message_notification/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/message_notification/table','set') || $check_action('/message_notification/view','set') || $check_action('/message_notification/view','get')" class="btn btn-primary">详情</navigator>
	    					</view>
				</view>
			</view>
		</view>
