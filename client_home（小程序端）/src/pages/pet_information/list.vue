<template>
	<view class="page_pet_information diy_list diy_list_page div_list_tml" id="pet_information_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				...
				<view>
					<view>
						<view class="tab_view">
							<!-- 筛选栏 -->
														<uni-section title="宠物类型" type="line">
							<uni-data-select
									@change="change_type_of_pet"
									v-model="query['type_of_pet']"
									:localdata="list_type_of_pet"
							></uni-data-select>
							</uni-section>
																							<uni-section title="领养状态" type="line">
							<uni-data-select
									@change="change_adoption_status"
									v-model="query['adoption_status']"
									:localdata="list_adoption_status"
							></uni-data-select>
							</uni-section>
																				<uni-section title="是否疾病" type="line">
							<uni-data-select
									@change="change_disease_or_not"
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?pet_information_id=' + o['pet_information_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
		...
		data() {
			return {
				url_get_list: "~/api/pet_information/get_list?like=0",
							// 宠物类型选项列表
				list_type_of_pet: [{value:"全部",text:"全部"}],
									// 宠物性别选项列表
				list_pet_sex: [{value:"全部",text:"全部"}],
											// 领养状态选项列表
				list_adoption_status: [{value:"全部",text:"全部"}],
									// 是否疾病选项列表
				list_disease_or_not: [{value:"全部",text:"全部"}],
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=pet_information", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["pet_information_id"] === o["source_id"]){
				...
				console.error(res.error);
						...
						// 改变分类标签
	change_type_of_pet(val) {
		if (val === "全部" || val === "") {
			this.query.type_of_pet = "";
		}else{
			this.query.type_of_pet = val
		}
		this.search();
	},
	...
	async get_list_pet_sex() {
					['公','母'].map((o) => this.list_pet_sex.push({value:o,text:o}));
						...
						// 改变分类标签
	change_adoption_status(val) {
		if (val === "全部" || val === "") {
			this.query.adoption_status = "";
		}else{
			this.query.adoption_status = val
		}
		this.search();
	},
	...
	async get_list_adoption_status() {
					['待领养','已被领'].map((o) => this.list_adoption_status.push({value:o,text:o}));
						...
						// 改变分类标签
	change_disease_or_not(val) {
		if (val === "全部" || val === "") {
			this.query.disease_or_not = "";
		}else{
			this.query.disease_or_not = val
		}
		this.search();
	},
	 ...
	 */
	to_nav(id) {
		this.$nav('/pages/pet_information/details?pet_information_id=' + id)
	},
