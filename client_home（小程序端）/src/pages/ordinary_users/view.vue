          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','user_name') || $check_field('add','user_name') || $check_field('set','user_name')" label="用户姓名" name="user_name">
                                <uni-easyinput type="text" v-model="form['user_name']" v-if="user_group === '管理员' || (form['ordinary_users_id'] && $check_field('set','user_name')) || (!form['ordinary_users_id'] && $check_field('add','user_name'))" :disabled="disabledObj['user_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','user_gender') || $check_field('add','user_gender') || $check_field('set','user_gender')" label="用户性别" name="user_gender">
                  ...
                  v-if="user_group === '管理员' || (form['ordinary_users_id'] && $check_field('set','user_gender')) || (!form['ordinary_users_id'] && $check_field('add','user_gender'))"
                ></uni-data-select>
                <!-- 仅查看 -->
...
export default {
  ...
  data() {
    return {
      ...
      url_get_obj: "~/api/ordinary_users/get_obj?",
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/ordinary_users/upload?'),
