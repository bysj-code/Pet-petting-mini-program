<template>
  <view class="page_message_feedback diy_table diy_table_tml" id="message_feedback_table">
    <!-- 筛选模块(开始) -->
    <view>
          ...
          <view>
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    ...
                    <uni-forms-item label="留言标题" name="message_title">
                      <uni-easyinput type="text" v-model="query.message_title" placeholder="留言标题" />
                    </uni-forms-item>
                    <uni-forms-item label="留言日期" name="message_date">
                      <uni-easyinput type="text" v-model="query.message_date" placeholder="留言日期" />
                    </uni-forms-item>
                  </uni-forms>
              <!-- /搜索栏 -->
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/message_feedback/view?" v-show="user_group == '管理员' || $check_action('/message_feedback/table','add') || $check_action('/message_feedback/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="0 && $check_field('get','message_user')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>留言用户</span>
								...
								<text>
									{{ get_user_message_user(o['message_user']) }}
								</text>
							</view>
										</view>
					</view>
                            ...
                            <span>用户姓名</span>
								...
								<text>
									{{ o["user_name"] }}
													...
													<view v-if="1 && $check_field('get','message_title')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>留言标题</span>
                          </view>
								...
								<text>
									{{ o["message_title"] }}
													...
													<view v-if="0 && $check_field('get','message_date')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>留言日期</span>
                          </view>
								...
								<text>
									{{ $toTime(o["message_date"],"yyyy-MM-dd") }}
													...
													<view v-if="0 && $check_field('get','message_content')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>留言内容</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/message_feedback/table','del') || $check_action('/message_feedback/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/message_feedback/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/message_feedback/table','set') || $check_action('/message_feedback/view','set') || $check_action('/message_feedback/view','get')" class="btn btn-primary">详情</navigator>
	    					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/message_feedback/get_list?like=0",
				url_del: "~/api/message_feedback/del?",
		...
		methods: {
          ...
          reset: function reset() {
            ...
            if(user_group != "管理员"){
                switch(user_group){
                                case "普通用户":
                  param["message_user"] = this.user.user_id;
                  break;
                                                                                          }
        }
    return param;
